...
public abstract class EventsServiceBase : IEventsService
{
    ...
    public async Task<Event> CreateEvent(EventCreateInput createDto)
    {
        ...
        {
            eventDbModel.Id = createDto.Id;
        }
        if (createDto.Feedbacks != null)
        {
            eventDbModel.Feedbacks = await _context
        ...
        {
            eventDbModel.Notifications = await _context
        ...
        {
            eventDbModel.ParticipantRegistrations = await _context
        ...
        {
            eventDbModel.Sessions = await _context
        ...
        var result = await _context.FindAsync<EventDbModel>(eventDbModel.Id);
    ...
    {
        var eventDbModel = await _context.Events.FindAsync(uniqueId.Id);
        if (eventDbModel == null)
        {
            throw new NotFoundException();
        }

        _context.Events.Remove(eventDbModel);
        await _context.SaveChangesAsync();
    }
    ...
    public async Task<List<Event>> Events(EventFindManyArgs findManyArgs)
    {
        var events = await _context
            ...
            .ToListAsync();
        return events.ConvertAll(eventDbModel => eventDbModel.ToDto());
    }
    ...
    {
        var count = await _context.Events.ApplyWhere(findManyArgs.Where).CountAsync();
    ...
    public async Task<Event> Event(EventWhereUniqueInput uniqueId)
    {
        ...
        var eventDbModel = events.FirstOrDefault();
        if (eventDbModel == null)
        {
            throw new NotFoundException();
        }
    ...
    {
        var eventDbModel = updateDto.ToModel(uniqueId);
        ...
        _context.Entry(eventDbModel).State = EntityState.Modified;
        ...
        {
            if (!_context.Events.Any(e => e.Id == eventDbModel.Id))
            {
                throw new NotFoundException();
            }
            else
    ...
    public async Task<List<Feedback>> FindFeedbacks(
        EventWhereUniqueInput uniqueId,
        FeedbackFindManyArgs eventFindManyArgs
    )
    {
        var feedbacks = await _context
            .Feedbacks.Where(m => m.EventId == uniqueId.Id)
    ...
    {
        var eventDbModel = await _context
        ...
        eventDbModel.Feedbacks = children;
        await _context.SaveChangesAsync();
    }
    ...
    public async Task<List<Notification>> FindNotifications(
        EventWhereUniqueInput uniqueId,
        NotificationFindManyArgs eventFindManyArgs
    )
    {
        var notifications = await _context
            .Notifications.Where(m => m.EventId == uniqueId.Id)
            .ApplyWhere(eventFindManyArgs.Where)
            .ApplySkip(eventFindManyArgs.Skip)
            .ApplyTake(eventFindManyArgs.Take)
            .ApplyOrderBy(eventFindManyArgs.SortBy)
    ...
    )
    {
        ...
        eventDbModel.Notifications = children;
        await _context.SaveChangesAsync();
    }
    ...
    public async Task<List<ParticipantRegistration>> FindParticipantRegistrations(
        EventWhereUniqueInput uniqueId,
        ParticipantRegistrationFindManyArgs eventFindManyArgs
    )
    {
        var participantRegistrations = await _context
            .ParticipantRegistrations.Where(m => m.EventId == uniqueId.Id)
            .ApplyWhere(eventFindManyArgs.Where)
            .ApplySkip(eventFindManyArgs.Skip)
            .ApplyTake(eventFindManyArgs.Take)
            .ApplyOrderBy(eventFindManyArgs.SortBy)
    ...
    )
    {
        ...
        eventDbModel.ParticipantRegistrations = children;
        await _context.SaveChangesAsync();
    }
    ...
    public async Task ConnectSessions(
        EventWhereUniqueInput uniqueId,
        SessionWhereUniqueInput[] childrenIds
    )
    {
        var parent = await _context
            .Events.Include(x => x.Sessions)
    ...
    public async Task<List<Session>> FindSessions(
        EventWhereUniqueInput uniqueId,
        SessionFindManyArgs eventFindManyArgs
    )
    {
        var sessions = await _context
            .Sessions.Where(m => m.EventId == uniqueId.Id)
            .ApplyWhere(eventFindManyArgs.Where)
            .ApplySkip(eventFindManyArgs.Skip)
            .ApplyTake(eventFindManyArgs.Take)
            .ApplyOrderBy(eventFindManyArgs.SortBy)
    ...
    )
    {
        ...
        eventDbModel.Sessions = children;
        await _context.SaveChangesAsync();
    }
}
